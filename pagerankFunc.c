// A helper file to go with pageranking
// Created 18/10/17
...
Set PageRank(float d, float diffPR, int maxIterations) {
	...
	//printf("Fetching outbound links...\n");
	...
	*/
	...
	while (iteration < maxIterations && diff >= diffPR) {
		...
		for (int i = 0; i < elements; i++) {
			...
			//printf("Elements %d   d %f   Wval %f\n", elements, d, weightedWValue);
...
*/
	...
	for (int i = 0; i < nElems(rankSet); i++) {
		...
		float rank = retrieveRank(rankSet, i);
		fprintf(pagerankfp, "%s, %.0f, %.7f\n", temp, outbound[i], rank);
	}
